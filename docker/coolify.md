...
3. **Env vars** (see `.env.example`): `CLIENT_SLUG, DATABASE_URL, NEXT_PUBLIC_SITE_URL, STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY, RESEND_API_KEY, RESEND_FROM, TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM, KITCHEN_PIN, ADMIN_PASSWORD, LAUNCHFLOW_KEY, SESSION_SECRET, CRON_SECRET`.
